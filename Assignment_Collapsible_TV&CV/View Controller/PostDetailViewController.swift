...
class PostDetailViewController: BaseViewController {
    ...
    override func viewDidLoad() {
        ...
        setupNavigationBar(withTitle: "Post Details", withDropdown: false)
    ...
    private func fetchComments() {
        ...
        DispatchQueue.global().asyncAfter(deadline: .now() + 2) { [weak self] in
            // Simulated comments (fetching comments is required for data update, but can be adjusted if not needed)
...
extension PostDetailViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.section == 1 { // Comments section
            ...
            tableView.reloadSections(IndexSet(integer: 1), with: .automatic) // Reload comments section (no functionality change)
        }
    }
